...
| Seed templates | Five global templates and initial DAG edges in `0001_init.sql`; static DAG audit checks acyclic edges; migration applied in DB Spike run `25416647604` | Done |
...
| Signup bootstrap routes | `apps/web/app/api/invite/[code]/route.ts`, `apps/web/app/api/auth/bootstrap/route.ts`; V7/V8 DB harness passed in run `25416647604` | Done for DB semantics; HTTP E2E not run |
| Worker long process skeleton | `apps/worker/src/solo_agent_worker/main.py` | Done |
| Worker jobs table pickup | `apps/worker/src/solo_agent_worker/db.py` uses `FOR UPDATE SKIP LOCKED`; V10 DB harness passed in run `25416647604` | Done |
...
| DB runtime CI | `.github/workflows/db-spike.yml` run `25416647604` on branch `stage1-agent-platform-verify` | Passed |
...
| DB V7 | `pnpm spike:db-all` in DB Spike run `25416647604` | Passed |
| DB V8 | `pnpm spike:db-all` in DB Spike run `25416647604` | Passed |
| DB V9 | `pnpm spike:db-all` in DB Spike run `25416647604` | Passed for local Postgres LISTEN/NOTIFY and replay; Vercel Pro runtime not run |
| DB V10 | `pnpm spike:db-all` in DB Spike run `25416647604` | Passed |
| DB V11 | `pnpm spike:db-all` in DB Spike run `25416647604` | Passed |
...
- GitHub Actions DB Spike run: `25416647604`
