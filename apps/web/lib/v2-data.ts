...
export async function getAgentDetail(agentId: string): Promise<AgentDetail | null> {
  try {
    return await withDb(async (client) => {
      const agentRes = await client.query<{ id: string; name: string; role: string; kind: string; status: string; system_prompt: string | null; proxy_agent_id: string | null; monthly_budget: string | null }>(
        `SELECT id::text, name, role, kind, status, system_prompt, proxy_agent_id::text, monthly_budget::text FROM virtual_agents WHERE id = $1::uuid`,
        [agentId],
      );
...
export async function updateAgent(
  id: string,
  fields: { system_prompt?: string; role?: string; proxy_agent_id?: string | null; monthly_budget?: number | null },
): Promise<boolean> {
    ...
    return await withDb(async (client) => {
      const result = await client.query<{ id: string }>(
        `INSERT INTO assignments
           (desk_id, title, prompt, assigned_to_agent_id, status, origin)
         VALUES ($1, $2, $3, $4::uuid, 'queued', $5)
         RETURNING id::text`,
        [CEO_DESK_ID, title, prompt, assignedToAgentId, origin],
      );
      return result.rows[0] ?? null;
    });
  } catch {
